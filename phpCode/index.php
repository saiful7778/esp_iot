...
$dBUsername = "id20113387_saifulislam7778";
$dBPassword = "XViE&Y2rdrrvmSwM";
$dBName = "id20113387_esp32iot";
...
if (isset($_POST['toggle_LED'])) {
	$sql = "SELECT * FROM relay_switch;";
	$result   = mysqli_query($conn, $sql);
	$row  = mysqli_fetch_assoc($result);
	
	if($row['status'] == 0){
		$update = mysqli_query($conn, "UPDATE relay_switch SET status = 1 WHERE id = 1;");		
	}		
	else{
		$update = mysqli_query($conn, "UPDATE relay_switch SET status = 0 WHERE id = 1;");		
	}
}
